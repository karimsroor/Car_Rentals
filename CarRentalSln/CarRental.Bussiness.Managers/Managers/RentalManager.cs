...
namespace CarRental.Bussiness.Managers
{
    ...
    public class RentalManager : ManagerBase, IRentalService
    {
       ...
       IDataRepositoryFactory _DataRepositoryFactory ;
        [Import]
       IBusinessEngineFactory _BussinessEngineFactory;
        ...
        public RentalManager(IDataRepositoryFactory dataRepositoryFactory,IBusinessEngineFactory bussinessEngineFactory)
        {
            _DataRepositoryFactory = dataRepositoryFactory;
            _BussinessEngineFactory = bussinessEngineFactory;
        }
        ...
        protected override Account LoadAuthorizationValidationAccount(string loginName)
        {
            ...
            if (authAcc == null)
            {
                NotFoundException ex = new NotFoundException(string.Format("cannot find account for LoginName {0} to use for security validation check",loginName));
        ...
        public Rental RentCarToCustomer(string loginEmail, int carId, DateTime dateDueBack)
        {
            return ExecuteFaultHandledOperation(() =>
            {
                ICarRentalEngine carRentalEngine = _BussinessEngineFactory.GetBusinessEngine<ICarRentalEngine>();
